    ...
    public Sendable processMail(Sendable mail) {
        if (mail.getClass() == MailMessage.class) {
            ...
            if (from.equals(AUSTIN_POWERS) || to.equals(AUSTIN_POWERS)) {
                ...
                LOGGER.warning(new StringBuilder().append("Detected target mail correspondence: from ").append(from).append(" to ").append(to).append(" \"").append(mailMessage.getMessage()).append("\"").toString());

            } else {
                //LOGGER.info("Usual correspondence: from " + from + " to " + to + "");
                LOGGER.info(new StringBuilder().append("Usual correspondence: from ").append(from).append(" to ").append(to).append("").toString());
            }
        }
        return mail;
    }
